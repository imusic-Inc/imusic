...
function MySession() {
 ...
 async function init(id) {
    let token = await APIController.getToken();
     const genes = await APIController.getTracks(token, id);
     setGenre({...genes});
     
 }
    
    async function initPlaylist(id) {
    let token = await APIController.getToken();
     const genes = await APIController.getPlayList(token, id);
     setplayList([...genes]);
  ...
  return (
      ...
      <div className="flex-6 left-20 ">
              ...
              {genre.description ? <PlayListHead values={genre} />:<></>}  
...
<table className="p-2">
    <tr>
        ...
        <th className="opacity-6 text-left">DATE ADDED</th>
